...
SEXP sample_power_post_S_b_C(SEXP RY, SEXP RA, SEXP Rtaus, SEXP Rbetas, SEXP Rb, SEXP RSold, SEXP Rpwr, SEXP RNsim, SEXP RS)
{
    /* Digest the datastructures (SEXPs) from R */
    int Nsim, n;
    double *Y, *A, *taus, *betas, *b, *Sold, *S, pwr;
    ...
    n = length(RY);
    pwr = REAL(coerceVector(Rpwr,REALSXP))[0];
    ...
    for (j=1; j<Nsim; j++) {
	for (i=0; i<n; i++) {
	    log_accept = pwr*(dpois(Y[i],A[i]*S[j+i*Nsim]+b[i],1)-dpois(Y[i],A[i]*sam[j-1+i*Nsim]+b[i],1));
...
/*
    ...
    for (j in 2:Nsim){
        ...
        log_U <- log(runif(n))
        sam[j,] <- ifelse(log_U<=log_accept,S[i,],sam[j-1,])
    }
    return(sam)
*/
